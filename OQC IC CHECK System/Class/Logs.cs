...
namespace OQC_IC_CHECK_System
{
    ...
    public sealed class Logs
    {
        ...
        private Logs()
        {
            ...
            Thread runDeleteLOG = new Thread(ThreadDeleteLOG);
            runDeleteLOG.IsBackground = true;
            runDeleteLOG.Start();
        }

        private void ThreadDeleteLOG()
        {
            while (true)
            {
                try
                {
                    Thread.Sleep(10000);
                    string DirectoryPath = Application.StartupPath + FolderName;
                    string ErrorDirectoryPath = Application.StartupPath + ErrorFolderName;
                    string[] dir = Directory.GetFiles(DirectoryPath);
                    string[] dir2 = Directory.GetFiles(ErrorDirectoryPath);
                    List<string> dirs = new List<string>();
                    dirs.AddRange(dir.ToList());
                    dirs.AddRange(dir2.ToList());
                    foreach (string file in dirs)
                    {
                        FileInfo fileinfo = new FileInfo(file);
                        TimeSpan span = new TimeSpan();
                        span = DateTime.Now - fileinfo.CreationTime;
                ...
                {
                    Console.WriteLine("删除日志异常:"+ex.ToString());
                }
            }
        }
